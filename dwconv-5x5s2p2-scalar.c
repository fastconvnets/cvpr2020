...
void f32_dwconv_spchw_ukernel_5x5s2p2__scalar(
    size_t m,
    size_t n,
    const float* input,
    const float* weights,
    ...
    const union f32_spchw_params params[restrict static 1])
{
  ...
  const size_t input_width_increment_single = input_width_stride * 2 - (1 + 2 * ((n - 1) / 2)) * input_tuple_stride;
  const size_t output_width_increment_single = output_width_stride - (n - 1) / 2 * output_tuple_stride;
