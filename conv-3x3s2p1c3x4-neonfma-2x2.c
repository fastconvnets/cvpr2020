// Copyright 2019 The Fast Sparse ConvNets Authors.
//
...
void f32_conv_hwc2spchw_ukernel_3x3s2p1c3x4__neonfma_2x2(
    size_t input_height,
    size_t input_width,
    ...
    const union f32_output_params params[restrict static 1])
{
  ...
  if (output_y_start < input_padding_top) {
    i0 = zero;
  }
  ...
  for (size_t output_y = output_y_start; output_y < output_y_end; output_y += 2) {
    ...
    if (input_y2 >= input_height) {
      i2 = zero;
    }
    if (input_y4 > input_height) {
      i3 = zero;
    }
    if (input_y4 >= input_height) {
      i4 = zero;
    }
    if (output_y + 2 > output_y_end) {
      output1 = output0;
    }
